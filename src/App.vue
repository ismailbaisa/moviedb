...
<div id="app">
    <detail v-if="modal">
      ...
      <span class="likes flex flex-ai-c flex-jc-e">
            <span>{{ detail.like}}</span><span class="material-icons love">
                favorite
            </span>
        </span>
    </detail>
    <div class="movies flex flex-jc-c flex-ai-c flex-fd-c" :class="turn">
        ...
        <div class="movies__list flex flex-jc-c flex-ai-c">
            <div v-for="movie in movies" :key="movie.id" @click="getDetail(movie)" class="flex flex-jc-sb flex-ai-c flex-fd-c">
                <h3>{{ movie.title }}</h3>
                <img lazy-load :src="movie.image" />
                <h4>SHOW TIME</h4>
                <h4>{{ movie.showTime }}</h4>
                <span class="likes flex flex-ai-c flex-jc-e">
                    <span>{{ movie.like}}</span><span class="material-icons love">
                        favorite
                    </span>
                </span>
            </div>
